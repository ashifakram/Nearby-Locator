...
// Sliding-window IP Rate Limiter with Bounded In-Memory Fallback
export const ipRateLimiter = (limit = 60, windowSeconds = 60, namespace = 'global') => {
  return async (req, res, next) => {
    ...
    try {
      ...
      dbLogger.error('IP Rate Limiter failed, using bounded local in-memory fallback', err);
      const ip = req.ip;
      const now = Date.now();
...
export const checkAccountLockout = async (req, res, next) => {
  try {
    ...
    dbLogger.error('Lockout middleware check failed, falling back to static throttle delay', err);
